...
def get_parser():
    ...
    parser.add_argument('--notes', default="ResnetAMCA_DomClas_GAN")
    return parser
...
class Discriminator(tf.keras.Model):
  def __init__(self, num_hidden, num_classes, activation='relu'):
    ...
    self.hidden_layers = []
    self.hidden_layers.append(tf.keras.layers.Dense(num_hidden, activation=activation))
...
def train_step(src_data,
    ...
    disc_gradients = disc_tape.gradient(batch_domain_loss, disc.trainable_variables)
