...
def virtual_adversarial_images(images, logits, pert_norm_radius=3.5):
    with tf.GradientTape() as tape:
        ...
        tape.watch(noise)
        noise = 1e-6 * tf.nn.l2_normalize(noise,
                                          axis=tf.range(1, len(noise.shape)))

        # Add noise to image and get new logits
        noise_logits, _ = model(images + noise, training=False)

        # Get loss from noisey logits
        noise_loss = tf.nn.softmax_cross_entropy_with_logits(
            labels=logits, logits=noise_logits)
...
def train_step(src_images, src_labels, s, m):
    with tf.GradientTape() as tape:
        ...
        src_adver_images = virtual_adversarial_images(src_images,
                                                      tf.nn.softmax(src_logits))
        src_adver_logits, _ = model(tf.stop_gradient(src_adver_images),
                                    training=True)
        src_adver_logits = AM_logits(labels=src_labels,
                                     logits=src_adver_logits,
                                     m=m,
                                     s=s)
        batch_VAT_cross_entropy_loss = get_cross_entropy_loss(
            labels=src_labels, logits=src_adver_logits)
    ...
    for epoch in range(epochs):
        ...
        with summary_writer.as_default():
            ...
            tf.summary.scalar("cross_entropy_loss",
                              ...
                              step=epoch)
            tf.summary.scalar("VAT_cross_entropy_loss",
                              VAT_cross_entropy_loss.result(),
                              step=epoch)
