...
def get_parser():
    ...
    parser.add_argument('--model_filters', type=int, default=16)
    parser.add_argument('--activation_fn', default='selu')
    parser.add_argument('--gpu', default='0')
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--num_classes', type=int, default=10)
    parser.add_argument('--train_source_days', type=int, default=2)
    ...
    parser.add_argument('--VAT_lambda', type=float, default=1e-1)
    parser.add_argument('--log_dir', default="logs/Baselines/AMCA_VAT/")
    parser.add_argument('--notes', default="AMCA_VAT_Baseline")
    return parser
...
def virtual_adversarial_images(images, logits, pert_norm_radius=3.5):
  with tf.GradientTape() as tape:
    ...
    noise_logits, _ = model(images + noise, training=False)

    # Get loss from noisey logits
    noise_loss = tf.nn.softmax_cross_entropy_with_logits(labels=logits,
                                                         logits=noise_logits)
    noise_loss = tf.reduce_mean(noise_loss)

  # Based on perturbed image loss, get direction of greatest error
  adversarial_noise = tape.gradient(noise_loss, noise)
...
def train_step(src_images, src_labels, s, m):
  with tf.GradientTape() as tape:
    ...
    src_adver_logits, _ = model(tf.stop_gradient(src_adver_images), training=True)
    src_adver_logits    = AM_logits(labels=src_labels, logits=src_adver_logits, m=m, s=s)
    batch_VAT_cross_entropy_loss  = get_cross_entropy_loss(labels=src_labels,
                                                           logits=src_adver_logits)

    batch_total_loss = batch_cross_entropy_loss + \
                       VAT_lambda * batch_VAT_cross_entropy_loss
  ...
  VAT_cross_entropy_loss(batch_VAT_cross_entropy_loss)
...
if __name__=='__main__':
    ...
    VAT_cross_entropy_loss = tf.keras.metrics.Mean(name='VAT_cross_entropy_loss')
    ...
    for epoch in range(epochs):
      ...
      with summary_writer.as_default():
        ...
        tf.summary.scalar("VAT_cross_entropy_loss", VAT_cross_entropy_loss.result(), step=epoch)
        tf.summary.scalar("total_loss", total_loss.result(), step=epoch)
      ...
      VAT_cross_entropy_loss.reset_states()
