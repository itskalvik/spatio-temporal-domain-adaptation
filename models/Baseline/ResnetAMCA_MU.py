...
def train_step(src_images, src_labels, s, m):
    with tf.GradientTape() as tape:
        ...
        batch_mu_cross_entropy_loss = get_cross_entropy_loss(
            labels=mu_src_labels, logits=mu_src_logits)
    ...
    for epoch in range(epochs):
        ...
        with summary_writer.as_default():
            ...
            tf.summary.scalar("cross_entropy_loss",
                              ...
                              step=epoch)
            tf.summary.scalar("mu_cross_entropy_loss",
                              mu_cross_entropy_loss.result(),
                              step=epoch)
