...
def train_step(src_images, src_labels, s, m):
  with tf.GradientTape() as tape:
    ...
    mu_src_images, mu_src_labels = mixup(src_images, src_labels, alpha=1)
    mu_src_logits, _ = model(mu_src_images, training=True)
    batch_mu_cross_entropy_loss  = get_cross_entropy_loss(labels=mu_src_labels,
                                                          logits=mu_src_logits)

    total_loss = batch_cross_entropy_loss + \
                 mu_lambda * batch_mu_cross_entropy_loss

  gradients = tape.gradient(total_loss, model.trainable_variables)
  optimizer.apply_gradients(zip(gradients, model.trainable_variables))

  source_train_acc(src_labels, tf.nn.softmax(src_logits))
  mu_cross_entropy_loss(batch_mu_cross_entropy_loss)
...
if __name__=='__main__':
    ...
    mu_cross_entropy_loss = tf.keras.metrics.Mean(name='mu_cross_entropy_loss')
    ...
    for epoch in range(epochs):
      ...
      with summary_writer.as_default():
        ...
        tf.summary.scalar("mu_cross_entropy_loss", mu_cross_entropy_loss.result(), step=epoch)
      ...
      mu_cross_entropy_loss.reset_states()
