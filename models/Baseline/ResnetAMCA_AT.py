...
def get_parser():
    ...
    parser.add_argument('--AT_lambda', type=float, default=1e-1)
    parser.add_argument('--log_dir', default="logs/Baselines/AMCA_AT/")
    parser.add_argument('--notes', default="AMCA_AT_Baseline")
    return parser
...
def train_step(src_images, src_labels, s, m):
  with tf.GradientTape() as tape:
    ...
    batch_AT_cross_entropy_loss = get_cross_entropy_loss(labels=src_labels,
                                                         logits=src_adver_logits)

    batch_total_loss = batch_cross_entropy_loss + \
                       AT_lambda * batch_AT_cross_entropy_loss
  ...
  AT_cross_entropy_loss(batch_AT_cross_entropy_loss)
...
if __name__=='__main__':
    ...
    AT_cross_entropy_loss  = tf.keras.metrics.Mean(name='AT_cross_entropy_loss')
    ...
    for epoch in range(epochs):
      ...
      with summary_writer.as_default():
        ...
        tf.summary.scalar("AT_cross_entropy_loss", AT_cross_entropy_loss.result(), step=epoch)
      ...
      AT_cross_entropy_loss.reset_states()
