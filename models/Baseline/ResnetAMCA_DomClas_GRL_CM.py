...
def get_parser():
    ...
    parser.add_argument('--notes', default="AMCA_DomClas_GRL_CM-src-trg-sep_Baseline")
    return parser
...
@tf.function
def train_step(src_images, src_labels, srv_images, srv_labels, s, m, hp_lambda=0):
  with tf.GradientTape() as tape:
    src_logits, src_dom_logits = model(src_images, training=True, hp_lambda=hp_lambda)
    ...
    srv_logits, srv_dom_logits = model(srv_images, training=True, hp_lambda=hp_lambda)
...
if __name__=='__main__':
    ...
    cm_loss               = tf.keras.metrics.Mean(name='cm_loss')
    ...
    for epoch in range(epochs):
      ...
      with summary_writer.as_default():
        ...
        tf.summary.scalar("cm_loss", cm_loss.result(), step=epoch)
