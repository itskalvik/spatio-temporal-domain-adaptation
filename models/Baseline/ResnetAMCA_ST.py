...
def get_parser():
    ...
    parser.add_argument('--notes', default="AMCA_CM_ST_Baseline")
    return parser
...
@tf.function
def train_step(src_images, src_labels, s, m, server=False):
    with tf.GradientTape() as tape:
        ...
        batch_cm_cross_entropy_loss = get_cross_entropy_loss(
            labels=cm_src_labels, logits=cm_src_logits)
    ...
    if server:
        server_train_acc(src_labels, tf.nn.softmax(src_logits))
    else:
...
if __name__ == '__main__':
    ...
    cm_cross_entropy_loss = tf.keras.metrics.Mean()
    ...
    for epoch in range(epochs):
        ...
        if epoch >= 2000:
            if epoch % 100 == 0:
                y_pseudo_train_server = []
                for data in X_train_server:
                    y_pseudo_train_server.extend(test_step(data))
                y_pseudo_train_server = np.array(y_pseudo_train_server)

                server_train_set = tf.data.Dataset.from_tensor_slices(
                    (X_train_server, y_pseudo_train_server))
                server_train_set = server_train_set.shuffle(
                    X_train_server.shape[0])
                ...
                with summary_writer.as_default():
                    tf.summary.image("Server Train Pseudo Confusion Matrix",
                                     cm_image,
                                     step=epoch)

            for server_data in server_train_set:
                train_step(server_data[0], server_data[1], s, m, server=True)
        ...
        with summary_writer.as_default():
            ...
            tf.summary.scalar("cross_entropy_loss",
                              ...
                              step=epoch)
            tf.summary.scalar("cm_cross_entropy_loss",
                              cm_cross_entropy_loss.result(),
                              step=epoch)
